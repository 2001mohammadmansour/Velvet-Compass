...
export async function createBooking({ hotelId, roomTypeId, checkIn, checkOut, specialRequests, guestName, guestCount, includeBreakfast, extraBedCount, paymentSenderWallet, paymentSenderName }) {
  ...
  const payload = {
    ...
    // Only set for Sham Cash bookings — the guest's own wallet/name, so the owner can match the
    // incoming payment against their Sham Cash app.
    paymentSenderWallet: paymentSenderWallet || null,
